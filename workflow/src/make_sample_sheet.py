...
def main():
    ...
    dmet.to_csv('config/deseq2_metadata.csv', index=False)
    ...
    output_base_dir = os.environ.get("OUTPUT_BASE_DIR", "data").rstrip("/")
    diffbind["bam"] = diffbind["sample"].apply(lambda x: f"{output_base_dir}/markd/" + x + ".sorted.markd.bam")
    diffbind["peaks"] = diffbind["mark"].apply(lambda x: f"{output_base_dir}/counts/" + x + "_consensus.bed")
