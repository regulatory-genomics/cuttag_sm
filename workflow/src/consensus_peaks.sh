...
# Use all sample BAMs; filenames do not include the mark identifier
...
ls ${OUTPUT_BASE_DIR}/Important_processed/Bam/*.sorted.markd.bam  | sed 's!.*/!!' | cut -d. -f1 | xargs |  tr ' ' '\t' | awk '{print "chrom\tstart\tend\t" $0}' | cat - ${OUTFILE}_tmp > ${OUTFILE}
