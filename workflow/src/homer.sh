...
contrasts=$(find "${OUTPUT_BASE_DIR}/deseq2/${mark}" -name "*.bed" | cut -d/ -f4 | sed -E "s/([a-zA-Z0-9.-]+)-($mark)-([a-zA-Z0-9-]+)(.bed)/\1/" | sort | uniq)
...
for contrast in $contrasts; do
	...
	up_05="${OUTPUT_BASE_DIR}/deseq2/${mark}/${contrast}-${mark}-differential-up-05.bed"
	up_01="${OUTPUT_BASE_DIR}/deseq2/${mark}/${contrast}-${mark}-differential-up-01.bed"
	down_05="${OUTPUT_BASE_DIR}/deseq2/${mark}/${contrast}-${mark}-differential-down-05.bed"
	down_01="${OUTPUT_BASE_DIR}/deseq2/${mark}/${contrast}-${mark}-differential-down-01.bed"
	...
	for file in ${differential_peaks[@]}; do
		...
		if [ -f "$file" ]; then
			...
			if [ "$peak_count" -ge 10 ]; then
				...
				# file I/O
				output_dir="${OUTPUT_BASE_DIR}/homer/${contrast}-${mark}-${directionality}-${sig}"
				log="${OUTPUT_BASE_DIR}/logs/homer_${contrast}-${mark}-${directionality}-${sig}.log"
				log_file_exists $log
...
wait
touch ${OUTPUT_BASE_DIR}/homer/$mark.done
