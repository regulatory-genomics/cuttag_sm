...
# read samplesheet to get sample, condition, and mark
st <- readr::read_tsv(params$samplesheet, show_col_types = FALSE)

# use sample column to match MultiQC keys and build labels via metadata
condition_rep_marks <- st$sample
mark_condition_reps <- paste(st$mark, st$condition, st$sample, sep = "_")
mark_conditions <- unique(paste(st$mark, st$condition, sep = "_"))
