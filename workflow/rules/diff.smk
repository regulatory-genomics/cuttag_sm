...
rule diffbind:
    ...
    shell:
        "Rscript workflow/src/diffbind.R -m {input.metadata} -c {input.consensus_peaks} -g {input.genes} -p {params.padj_cutoff} -o {params.outdir}"
...
rule diffbind_plots:
    ...
    singularity:
        os.path.join(config["SINGULARITY_IMAGE_FOLDER"], "diffbind.sif")
    shell:
        "Rscript workflow/src/diffbind_plots.R -i {input.rds_file} -p {params.padj_cutoff} -o {params.outdir}"
